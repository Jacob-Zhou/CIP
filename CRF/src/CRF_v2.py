...
class CRF(object):
    ...
    def forward(self, sentence):
        ...
        logalpha[0] = self.score(self.create_feature_template(sentence, 0, self.BOS))
        for i in range(1, length):
    ...
    def update_gradient(self, sentence):
        ...
        features = self.create_feature_template(sentence, 0, self.BOS)
        feature_index = [self.feature_space[feature] for feature in features]
        ...
        for i in range(1, length):
            unigram_features = self.create_unigram_feature(sentence, i)
            unigram_index = [self.feature_space[feature] for feature in unigram_features]
            ...
            for bigram_feature, prob in zip(self.bigram_features, probs):
                bigram_index = [self.feature_space[bi_f] for bi_f in bigram_feature]
        ...
        for iter in range(iterator):
            ...
            for sentence in self.train_data:
                ...
                if b == batch_size:
                    ...
                    b = 0
                    self.g = defaultdict(float)
                    self.bigram_scores = np.array([self.score(bigram_feature)
                                                   for bigram_feature in self.bigram_features])
