...
class global_linear_model(object):
    ...
    def create_unigram_feature(self, sentence, index, cur_tag):
        ...
        if len(words[index + 1]) == 1:
            unigram_feature.add(
                "12:" + cur_tag + "*" + words[index + 1] + "*" + words[index][-1] + "*" + words[index + 2][0])
    ...
    def create_feature_template(self, sentence, index, pre_tag, cur_tag):  # 创建某个tag相对于sentence句子中第index个词的特征
        feature_template = self.create_bigram_feature(pre_tag, cur_tag) | self.create_unigram_feature(sentence, index,
                                                                                                      cur_tag)
    ...
    def create_feature_space(self):
        ...
        self.feature_space = {feature: index for index, feature in
                              enumerate(self.feature_space_list)}  # 特征空间是一个字典，格式：{“NR*戴相龙:0”,"",....}
        self.E = len(self.feature_space)  # 特征空间的数目
        self.bigram_features = [[self.create_bigram_feature(pre_tag, cur_tag) for pre_tag in self.tags] for cur_tag in
                                self.tags]
    ...
    def predict(self, sentence, averaged=False):
        ...
        bigram_scores = np.array(
            [[self.get_score(_bigram_feature, averaged) for _bigram_feature in bigram_feature] for bigram_feature in
             self.bigram_features])
        for i in range(1, length):
            unigram_scores = np.array(
                [self.get_score(self.create_unigram_feature(sentence, i, tag), averaged) for tag in self.tags])
